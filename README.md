...
模型描述：
有一个套间，有一间客厅，有无数间卧室。每间卧室有一名住客。
客厅共用，卧室私有。

可以向住客发布任务。处理每个任务时，住客不会被中断，直到主动停止，开始或继续下一个任务。切换任务时，只需记录一个书签。
由于任务不会中断，所以紧急任务必须由专门住客执行。
由于同一住客有多个任务，所以有冲突的任务不得由同一住客执行。

客户物品被占用时，记录一个住客的书签。当物品占用过久时，任务分配给占用物品的住客。
